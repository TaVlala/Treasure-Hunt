// Stripe payment routes — ticket checkout + webhook + simple redirect pages.
// Checkout: POST /api/v1/stripe/checkout/:huntId (player auth, creates Checkout Session)
// Webhook:  POST /api/v1/stripe/webhook (raw body, mounted separately in index.ts)
// Pages:    GET  /api/v1/stripe/success|cancel (HTML, shown in the player's browser)
...
// Creates Payment + GameSession + PlayerProgress atomically; idempotent on replay.
export async function stripeWebhookHandler(req: Request, res: Response): Promise<void> {
  ...
  // Only handle checkout completion — acknowledge all other events
  ...
  if (!huntId || !playerId) {
    res.status(400).json({ error: 'Missing huntId/playerId in Stripe session metadata' });
    return;
  }

  try {
    // Idempotency: webhook may fire multiple times — skip if session already exists
    const existing = await prisma.gameSession.findUnique({
      where: { huntId_playerId: { huntId, playerId } },
    });
    if (existing) {
      res.status(200).json({ received: true, sessionId: existing.id });
      return;
    }

    // Load hunt for pricing info + clues for progress initialisation
    const hunt = await prisma.hunt.findUnique({
      where: { id: huntId },
    ...
    // Stripe payment_intent is the canonical charge ID; fall back to session ID
    const stripePaymentId =
      typeof checkoutSession.payment_intent === 'string'
        ? checkoutSession.payment_intent
        : checkoutSession.id;

    // Atomically: create GameSession + PlayerProgress + Payment (all linked)
    const gameSession = await prisma.$transaction(async (tx) => {
      ...
      await tx.payment.create({
        data: {
          ...
          stripePaymentId,
          status: 'COMPLETED',
          description: 'Hunt ticket purchase via Stripe Checkout',
          huntId,
          sessionId: session.id,
        },
      });
    ...
    res.status(200).json({ received: true, sessionId: gameSession.id });
  } catch (err) {
